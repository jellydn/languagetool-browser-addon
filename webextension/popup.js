...
function getCheckResult(markupList, metaData, callback, errorCallback) {
  ...
  req.onload = function() {
    ...
    if (!response) {
      errorCallback(
        chrome.i18n.getMessage("noResponseFromServer", serverUrl),
    ...
    if (req.status !== 200) {
      errorCallback(
        chrome.i18n.getMessage("noValidResponseFromServer", [
          serverUrl,
          req.response,
          req.status
        ]),
  ...
  req.onerror = function() {
    errorCallback(
      chrome.i18n.getMessage("networkError", serverUrl),
      "networkError"
    );
  };
  req.ontimeout = function() {
    errorCallback(
      chrome.i18n.getMessage("timeoutError", serverUrl),
      "timeoutError"
    );
  };
  ...
  let params =
    "disabledRules=WHITESPACE_RULE" + // needed because we might replace quoted text by spaces (see issue #25)
    "&useragent=" +
    userAgent;
  if (true) {
    // TODO: activate 'data' mode when server supports it
...
function renderMatchesToHtml(resultJson, response, tabs, callback) {
  ...
  let translatedLanguage = chrome.i18n.getMessage(
  ...
  Tools.getStorage().get(
    ...
    function(items) {
      ...
      for (let match in matches) {
        ...
        if (isSpellingError(m)) {
          ...
          if (knowToDict) {
            ...
            ignoreError =
              items.dictionary.indexOf(
                Tools.lowerCaseFirstChar(wordSanitized)
              ) != -1;
          }
        } else {
          ignoreError = items.ignoredRules.find(
            k => k.id === ruleIdSanitized && k.language === shortLanguageCode
          );
        }
        if (ignoreError) {
          ...
          if (isSpellingError(m)) {
            ...
            html +=
              "<div class='addToDict' data-addtodict='" +
              escapedWord +
              "'" +
              " title='" +
              chrome.i18n
                .getMessage("addToDictionaryTitle", escapedWord)
                .replace(/'/, "&apos;") +
              "'></div>";
          } else {
            html +=
              "<div class='turnOffRule' data-ruleIdOff='" +
              Tools.escapeHtml(ruleIdSanitized) +
              "'" +
              " data-ruleDescription='" +
              Tools.escapeHtml(descriptionSanitized) +
              "'" +
              " title='" +
              chrome.i18n.getMessage("turnOffRule").replace(/'/, "&apos;") +
              "'></div>";
          }
      ...
      if (quotedLinesIgnored) {
        html +=
          "<p class='quotedLinesIgnored'>" +
          chrome.i18n.getMessage("quotedLinesIgnored") +
          "</p>";
      }
      if (items.ignoredRules && items.ignoredRules.length > 0) {
        ...
        for (let key in items.ignoredRules) {
          ...
          if (currentLang === ignoredRule.language) {
            ...
            if (matchCount) {
              ruleItems.push(
                "<span class='ignoredRule'><a class='turnOnRuleLink' data-ruleIdOn='" +
                  ruleId +
                  "' href='#'>" +
                  ruleDescription +
                  " (" +
        ...
        if (ruleItems.length > 0) {
          html +=
            "<span class='ignoredRulesIntro'>" +
            chrome.i18n.getMessage("ignoredRules") +
            "</span> ";
...
function fillReviewRequest() {
  ...
  storage.get(
    ...
    function(items) {
      ...
      if (
        !items.reviewRequestLinkClicked &&
        items.usageCounter >= minUsageForReviewRequest
      ) {
        if (Tools.isChrome()) {
          const reviewRequestId = document.getElementById("reviewRequest");
          reviewRequestId.innerHTML = chrome.i18n.getMessage(
            "reviewRequest",
            thisExtensionUrl + "/reviews"
          );
...
function showShortcutHint(commands) {
  if (
    ...
    document.getElementById("outerShortcutHint").innerHTML =
      ...
      "&nbsp;<a id='closeShortcutHint' href='#'>" +
      chrome.i18n.getMessage("shortcutHintDismiss", [shortcut]) +
      "</a>" +
      "</div>";
    document
      .getElementById("closeShortcutHint")
...
function getLanguageSelector(languageCode) {
  ...
  html +=
    "<input type='hidden' id='prevLanguage' name='prevLanguage' value='" +
    Tools.escapeHtml(languageCode) +
    "'>";
  ...
  for (let l in languages) {
    ...
    if (!translatedLang) {
      translatedLang = chrome.i18n.getMessage(
        langCodeForTrans.replace(/_.*/, "")
      );
    }
    if (!translatedLang) {
  ...
  for (let idx in replacements) {
    ...
    if (createLinks) {
      html +=
        ...
        "'" +
        " data-errortext='" +
        Tools.escapeHtml(errorTextSanitized) +
        "'" +
        " data-replacement='" +
        Tools.escapeHtml(replacementSanitized) +
        "'" +
        "'>&nbsp;" +
        Tools.escapeHtml(replacementSanitized) +
        "&nbsp;</a>"; // add &nbsp; to make small links better clickable by making them wider
    } else {
...
function addListenerActions(elements, tabs, response) {
  for (let i = 0; i < elements.length; i++) {
    ...
    const isRelevant =
      link.getAttribute("data-ruleIdOn") ||
...
function handleCheckResult(response, tabs, callback) {
  ...
  if (Markup.markupList2text(response.markupList).trim() === "") {
    let msg =
      chrome.i18n.getMessage("noTextFound") +
      "<br>" +
      "<span class='errorMessageDetail'>" +
      chrome.i18n.getMessage("noTextFoundDetails") +
      "</span>";
    renderStatus(msg);
  ...
  getCheckResult(
    ...
    function(errorMessage, errorMessageCode) {
      renderStatus(
        chrome.i18n.getMessage(
          "couldNotCheckText",
          Tools.escapeHtml(DOMPurify.sanitize(errorMessage))
        )
      );
      Tools.logOnServer(
        "couldNotCheckText on " + tabs[0].url + ": " + errorMessageCode,
        serverUrl
      );
      if (callback) {
  ...
  Tools.getStorage().get(
    ...
    function(items) {
      ...
      if (items.allowRemoteCheck === true) {
        ...
        let message = "<p>";
        if (serverUrl === defaultServerUrl) {
          message += chrome.i18n.getMessage("privacyNoteForDefaultServer", [
            "https://languagetool.org",
            "https://languagetool.org/privacy/"
          ]);
        } else {
        ...
        message +=
          "<ul>" +
          '  <li><a class="privacyLink" id="confirmCheck" href="#">' +
          chrome.i18n.getMessage("continue") +
          "</a></li>" +
          '  <li><a class="privacyLink" id="cancelCheck" href="#">' +
